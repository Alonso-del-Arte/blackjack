...
public class WagerTest {
    ...
    @Test
    public void testPayoutFunctionInsuranceWon() {
        CurrencyAmount expected = chooseAmount();
        Wager instance = new Wager(expected);
        instance.settle(Wager.Outcome.INSURANCE_WON);
        CurrencyAmount actual = Wager.Outcome.INSURANCE_WON.payoutFunction
                .apply(expected);
        String message = "Payout for wager of " + expected.toString() 
                + " on insurance won";
        assertEquals(message, expected, actual);
    ...
    @Test
    public void testPayoutFunctionReplaced() {
        CurrencyAmount amount = chooseAmount();
        Wager instance = new Wager(amount);
        instance.settle(Wager.Outcome.REPLACED);
        CurrencyAmount expected = new CurrencyAmount(0, amount.getCurrency());
        CurrencyAmount actual = Wager.Outcome.REPLACED.payoutFunction
                .apply(amount);
        String message = "Payout for wager of " + amount.toString() 
                + " on replaced";
        assertEquals(message, expected, actual);
    ...
    @Test
    public void testPayoutFunctionInsuranceLost() {
        CurrencyAmount amount = chooseAmount();
        Wager instance = new Wager(amount);
        instance.settle(Wager.Outcome.INSURANCE_LOST);
        CurrencyAmount expected = amount.negate();
        CurrencyAmount actual = Wager.Outcome.INSURANCE_LOST.payoutFunction
                .apply(amount);
        String message = "Payout for wager of " + amount.toString() 
                + " on insurance lost";
        assertEquals(message, expected, actual);
